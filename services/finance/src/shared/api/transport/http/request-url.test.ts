...
function uriFor(config: { url: string; params?: Record<string, unknown> }): string {
	return axios
		.create({ baseURL: RESOURCE_BASE })
		.getUri({ ...config, paramsSerializer: { serialize: serializeQueryParameters } });
}
...
describe('serializeQueryParameters', () => {
	test('joins arrays with a comma rather than repeating the key', () => {
		expect(serializeQueryParameters({ target: ['RUB', 'EUR'] })).toBe('target=RUB%2CEUR');
	});

	test('drops absent values instead of sending empty ones', () => {
		expect(serializeQueryParameters({ limit: 8, cursor: undefined, severity: null })).toBe('limit=8');
	});

	test('drops an empty array rather than sending a blank key', () => {
		expect(serializeQueryParameters({ target: [] })).toBe('');
	});

	test('keeps a false boolean, which a metrics selector relies on', () => {
		expect(serializeQueryParameters({ 'net-worth': false })).toBe('net-worth=false');
	});
});
