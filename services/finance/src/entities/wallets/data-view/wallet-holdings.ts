...
const walletOwnedMoney = (wallet: Wallet): Money => ({
	amount: subtractAmounts(wallet.balance.amount, wallet.zeroBalance.amount),
...
const walletOwedMoney = (wallet: Wallet): Money => {
	...
	return {
		amount: compareAmounts(owned.amount, ZERO_AMOUNT) < 0
			? subtractAmounts(wallet.zeroBalance.amount, wallet.balance.amount)
			: ZERO_AMOUNT,
		currency: wallet.balance.currency,
	};
};
...
const walletIsSettled = (wallet: Wallet): boolean => {
	return isZeroAmount(subtractAmounts(wallet.balance.amount, wallet.zeroBalance.amount));
};
