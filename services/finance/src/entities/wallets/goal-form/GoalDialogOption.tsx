...
interface GoalDialogOptionProps extends Omit<ButtonHTMLAttributes<HTMLButtonElement>, 'className' | 'type' | 'role' | 'aria-checked'> {
	selected: boolean;
}
...
const GoalDialogOption: FC<GoalDialogOptionProps> = ({
	...
	<button
		...
		className={cn(
			"flex w-full items-start gap-3 rounded-[var(--radius-md)] border px-3.5 py-2.5 text-left transition-colors",
