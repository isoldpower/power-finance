...
const openCombobox = (value?: string, onSelected: (code: string) => void = vi.fn()) => {
	render(<CurrencyPicker currencies={CURRENCIES} value={value} onSelected={onSelected} />);
	fireEvent.click(screen.getByRole('combobox'));
};

describe('CurrencyPicker', () => {
	test('shows the selected currency on the trigger', () => {
		render(<CurrencyPicker currencies={CURRENCIES} value="EUR" onSelected={vi.fn()} />);

		expect(screen.getByRole('combobox')).toHaveTextContent('EUR · Euro');
	});

	test('shows only the code on the pill variant', () => {
		render(<CurrencyPicker currencies={CURRENCIES} value="EUR" variant="pill" onSelected={vi.fn()} />);
	...
	test('pivots the popover alignment', () => {
		render(<CurrencyPicker currencies={CURRENCIES} pivot="end" onSelected={vi.fn()} />);
		fireEvent.click(screen.getByRole('combobox'));
	...
	test('pivots the popover side', () => {
		render(<CurrencyPicker currencies={CURRENCIES} pivot="top" onSelected={vi.fn()} />);
		fireEvent.click(screen.getByRole('combobox'));
