...
const useWalletMutationsState = (id: string) => {
	const deletingMutations = useMutationState({
		filters: { mutationKey: [WALLETS_CACHE_KEYS.delete, id], status: 'pending' }
	});
	const updatingMutations = useMutationState({
		filters: { mutationKey: [WALLETS_CACHE_KEYS.update, id], status: 'pending' }
	});
	const replacingMutations = useMutationState({
		filters: { mutationKey: [WALLETS_CACHE_KEYS.replace, id], status: 'pending' }
	});
