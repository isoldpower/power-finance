...
// Dashboard quick-add behaviour: type/amount/wallet state, validation and the create/transfer
// submit. Presentation lives in the QuickAddPanel widget.
const useQuickAdd = () => {
	...
	const walletOptions: WalletSelectOption[] = wallets.map((wallet) => ({ id: wallet.id, name: wallet.name, currency: wallet.balance.currency, gradient: gradientFromId(wallet.id) }));
