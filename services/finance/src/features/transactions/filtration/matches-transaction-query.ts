...
const containsNeedle = (haystack: string | null, needle: string, caseSensitive: boolean): boolean => {
...
const matchesChain = (transaction: Transaction, chainId: string | undefined): boolean => {
	if (!chainId) return true;

	return transaction.chain?.id === chainId;
};

const matchesTypes = (transaction: Transaction, types: TransactionType[] | undefined): boolean => {
	if (!types?.length) return true;

	return types.includes(transaction.type);
};

const matchesText = (transaction: Transaction, query: TransactionQuery): boolean => {
...
const matchesTransactionQuery = (transaction: Transaction, query: TransactionQuery): boolean => (
