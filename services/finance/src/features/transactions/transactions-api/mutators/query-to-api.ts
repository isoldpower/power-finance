...
const transactionQueryToApi = (query: TransactionQuery): TransactionSearchBody => {
	...
	if (query.walletIds?.length) {
		leaves.push({ field_name: 'wallet_id', operator: 'in', value: query.walletIds });
	}
	if (query.chainId) {
		leaves.push({ field_name: 'chain_id', operator: 'eq', value: query.chainId });
	}
	if (query.currencies?.length) {
		leaves.push({ field_name: 'currency', operator: 'in', value: query.currencies });
	}
	if (query.minAmount !== undefined) {
		leaves.push({ field_name: 'amount', operator: 'gte', value: toAmountString(query.minAmount) });
	}
	if (query.maxAmount !== undefined) {
		leaves.push({ field_name: 'amount', operator: 'lte', value: toAmountString(query.maxAmount) });
	}
