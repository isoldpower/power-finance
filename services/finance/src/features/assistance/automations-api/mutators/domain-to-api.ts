...
const conditionToApi = (node: RuleNode): FilterNode => {
	...
	return { field_name: node.field, operator: node.operator, value: node.value };
};

const effectToApi = (effect: AutomationEffect): AutomationEffectDto => {
	switch (effect.type) {
		case 'set_category':
			return { type: effect.type, params: { category: effect.category } };
		case 'notify':
			return { type: effect.type, params: { severity: effect.severity, title: effect.title } };
		case 'raise_action':
			return {
				type: effect.type,
				params: { severity: effect.severity, title: effect.title, body: effect.body },
			};
		case 'transfer':
			return {
				type: effect.type,
				params: {
					...
					money: { amount: toAmountString(effect.money.amount), currency: effect.money.currency },
				},
			};
		case 'unknown':
			return { type: effect.name, params: {} };
	}
};

const triggerToApi = (trigger: AutomationTrigger): AutomationTriggerBody => ({
	type: trigger.type,
	...(trigger.type === 'event' && trigger.event ? { event: trigger.event } : {}),
	...(trigger.type === 'schedule' && trigger.schedule ? { schedule: trigger.schedule } : {}),
	filter_body: trigger.condition ? conditionToApi(trigger.condition) : null,
});
...
const automationPatchToApi = (patch: AutomationPatch): AutomationPatchBody => ({
	...
	...(patch.trigger === undefined ? {} : { trigger: triggerToApi(patch.trigger) }),
	...(patch.effects === undefined ? {} : { effects: patch.effects.map(effectToApi) }),
});
