...
import { listActions } from "@feature/actions";
import { CACHE_KEYS } from "./cache-config.ts";
import type { Action, ListActionsResponse } from "@feature/actions";
...
const useActions = (
	...
	const query = useQuery<ListActionsResponse>({
		queryKey: [CACHE_KEYS.list, params?.resolved ?? false, params?.limit ?? 'all'],
		queryFn: () => listActions({
